    ...
    <div className="min-h-screen font-mono selection:bg-primary selection:text-white">
      ...
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden pt-20 pb-32 mesh-bg">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center flex flex-col items-center gap-8">
          ...
          >
            Senior AI/ML Engineer specializing in LLMs and scalable Computer
            Vision architectures. // Turning complex mathematics into production
            software.
          </motion.p>
          ...
          >
            <motion.a
              whileHover={{
                scale: 1.05,
                boxShadow: "0 0 20px rgba(60, 131, 246, 0.4)",
              }}
              whileTap={{ scale: 0.95 }}
              href="#projects"
      ...
      <main className="max-w-7xl mx-auto px-6 py-24 space-y-48">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-16" id="about">
          ...
          >
            ...
            <p className="text-text-muted text-lg leading-relaxed font-mono">
              Tracing the evolution from neural network research to
          ...
          <div className="lg:col-span-8">
            <div className="relative pl-8 border-l border-slate-800 space-y-24">
              {EXPERIENCES.map((exp, i) => (
                ...
                >
                  <div
                    className={`absolute -left-[41px] top-0 size-5 rounded-full border-4 border-background-dark shadow-xl transition-all duration-300 ${
                      i === 0 ? "bg-primary shadow-primary/50" : "bg-slate-700"
                    }`}
                  ></div>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-2">
                    ...
                    <span className="text-[10px] text-text-muted font-bold tracking-widest bg-surface-dark border border-slate-800 px-3 py-1 rounded-full uppercase">
                      {exp.period}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.skills.map((skill, si) => (
                      <span
                        key={si}
                        className="px-3 py-1 rounded-md text-[10px] font-black bg-slate-900 text-slate-400 border border-slate-800 uppercase tracking-widest"
                      >
                        {skill}
                      </span>
                    ))}
        ...
        >
          ...
          <div className="p-8 md:p-12 flex flex-col lg:flex-row items-stretch gap-12">
            <div className="flex-1 space-y-8">
              <div>
                <h3 className="text-2xl md:text-4xl font-black text-white tracking-tighter uppercase italic">
                  Training_Convergence
                </h3>
              </div>
              <p className="text-text-muted leading-relaxed text-lg font-mono">
                Automated hyperparameter optimization pipeline. <br />
                // Maintaining validation stability while pushing training
                accuracy to 99.4%.
              </p>
        ...
        <section id="projects" className="space-y-16">
          ...
          >
            ...
            <p className="text-text-muted text-lg font-mono">
              Deep dives into production code and research implementations.
            </p>
          </motion.div>
        ...
        {/* Tech Stack Marquee */}
        <section
          className="space-y-16 relative z-20 overflow-hidden py-10"
          id="stack"
        >
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            <p className="text-text-muted text-lg font-mono">
              Technologies powering production systems.
            </p>
          </motion.div>
            ...
            <div className="overflow-hidden marquee-container">
              <div className="flex animate-marquee gap-12 w-max">
                {[...TECH_STACK, ...TECH_STACK].map((tech, i) => (
                  <div
                    key={i}
                    className="flex-shrink-0 flex items-center justify-center gap-4 px-8 py-6 rounded-2xl bg-surface-dark/50 border border-white/5 hover:border-primary/50 transition-colors"
                  >
                    <img
                      src={tech.logo}
                      alt={tech.name}
        ...
        >
          ...
          <div className="relative z-10 md:hidden overflow-hidden marquee-container">
            <div className="flex animate-marquee gap-6 w-max">
              {[...ENDORSEMENTS, ...ENDORSEMENTS].map((item, i) => (
                <div key={i} className="flex-shrink-0 w-[85vw] max-w-sm">
                  <TestimonialCard endorsement={item} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative z-10 hidden md:grid md:grid-cols-3 gap-8 h-[700px] vertical-mask">
            <div className="overflow-hidden h-full">
              <div className="animate-scroll-up flex flex-col gap-8">
                {[...ENDORSEMENTS.slice(0, 2), ...ENDORSEMENTS.slice(0, 2)].map(
                  (item, i) => (
            ...
            <div className="overflow-hidden h-full">
              <div className="animate-scroll-down flex flex-col gap-8">
                {[...ENDORSEMENTS.slice(2, 4), ...ENDORSEMENTS.slice(2, 4)].map(
                  (item, i) => (
                    <TestimonialCard key={i} endorsement={item} />
                  )
                )}
            ...
            <div className="overflow-hidden h-full">
              <div className="animate-scroll-up flex flex-col gap-8">
                {[...ENDORSEMENTS.slice(4, 6), ...ENDORSEMENTS.slice(4, 6)].map(
                  (item, i) => (
                    <TestimonialCard key={i} endorsement={item} />
                  )
                )}
        ...
        >
          ...
          <div className="relative bg-background-dark/90 backdrop-blur-3xl rounded-[36px] border border-white/10 p-16 md:p-32 overflow-hidden">
            <div className="relative z-10 max-w-3xl mx-auto space-y-10">
              <h2 className="text-2xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter leading-[1.1] italic uppercase">
                INIT_CONNECT?
              </h2>
              <p className="text-slate-400 text-xl leading-relaxed max-w-2xl mx-auto font-mono">
                Opening slots for Q4 collaboration and technical consulting.{" "}
                <br />
                // Establish handshake?
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-16 mb-20">
            <div className="col-span-1 md:col-span-2 space-y-6">
              <div className="flex items-center gap-2">
                <span className="font-bold text-2xl text-white tracking-tight font-mono uppercase">
                  JD.AI
                </span>
              </div>
      ...
      animate(0, stat.value, {
        ...
        onUpdate: (latest) => {
          if (counterRef.current) {
            counterRef.current.textContent = latest.toFixed(
              stat.value % 1 === 0 ? 0 : 1
            );
          }
        },
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[10px] font-black px-2 py-0.5 rounded text-primary bg-primary/20 hidden lg:block">
          {stat.sub}
        </span>
      </div>
...
const ProjectCard: React.FC<{ project: any; index: number }> = ({
  ...
  <motion.div
    ...
    transition={{ delay: index * 0.1 }}
    whileHover={{ y: -8 }}
    className={`group rounded-3xl bg-surface-dark border border-border-dark overflow-hidden flex flex-col transition-all duration-500 hover:border-primary/50 ${
      project.isWide ? "md:col-span-2" : ""
    } ${project.isHigh ? "md:row-span-2 h-full" : ""}`}
  >
    {project.image && (
      <div
        className={`relative overflow-hidden ${
          project.isHigh ? "flex-[4]" : "h-72"
        }`}
      >
        <img
          src={project.image}
    ...
    <div className={`p-8 flex flex-col ${project.isHigh ? "flex-1" : ""}`}>
      ...
      <h3 className="text-2xl font-black text-white mb-3 group-hover:text-primary transition-colors uppercase italic tracking-tight">
        {project.title}
      </h3>
      <p className="text-text-muted text-xs leading-relaxed mb-4 font-mono uppercase tracking-tight line-clamp-3">
        {project.description}
      </p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {project.tags?.map((tag: string) => (
          <span
            key={tag}
            className="text-[9px] font-black text-primary/80 border border-primary/20 px-2 py-0.5 rounded-full uppercase tracking-tighter"
          >
            #{tag}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
        <a
          className="text-[10px] font-black text-white hover:text-primary transition-colors inline-flex items-center gap-2 group/btn uppercase tracking-[0.2em]"
          href="#"
        >
          VIEW_CASE_STUDY
  ...
  <div className="glass-card p-8 rounded-[32px] hover:bg-white/5 transition-all duration-500 group relative">
    <div className="flex items-center gap-4 mb-8">
      <img
        ...
        className="size-14 rounded-full border-2 border-primary/20 group-hover:border-primary/50 transition-colors object-cover"
      />
      <div>
    ...
    <p className="text-slate-400 italic leading-relaxed text-xs font-mono">
      "{endorsement.text}"
    </p>
  </div>
);
